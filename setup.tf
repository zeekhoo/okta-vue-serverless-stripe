...
resource "okta_app_oauth" "mlmBlogApp" {
  ...
  type           = "browser"
  grant_types    = ["implicit"]
  redirect_uris  = ["https://${var.udp_subdomain}.bod-blog.unidemo.live/implicit/callback"]
  response_types = ["token", "id_token"]
  issuer_mode    = "ORG_URL"
  groups         = ["${data.okta_group.all.id}"]
  consent_method = "TRUSTED"
...
resource "okta_trusted_origin" "bodBlogUnidemo" {
  name   = "${var.udp_subdomain}.bod-blog.unidemo.live"
  origin = "https://${var.udp_subdomain}.bod-blog.unidemo.live"
  scopes = ["CORS"]
}
data "template_file" "bodConfiguration" {
  template = "${file("${path.module}/bod.dotenv.template")}"
  vars = {
    client_id         = "${okta_app_oauth.mlmDemoApp.client_id}"
    client_secret     = "${okta_app_oauth.mlmDemoApp.client_secret}"
    auth_server_id    = "${okta_auth_server.bodUnidemo.id}"
    domain            = "${var.org_name}.${var.base_url}"
    prospect_group_id = "${okta_group.prospect.id}"
    customer_group_id = "${okta_group.customer.id}"
    client2_id        = "${okta_app_oauth.mlmBlogApp.client_id}"
  }
}
resource "local_file" "bodDotenv" {
  content  = "${data.template_file.bodConfiguration.rendered}"
  filename = "${path.module}/bod.env"
}
